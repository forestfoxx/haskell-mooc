...
longestRepeat = maximum . (0 :) . map L.length . L.group . T.unpack
...
takeStrict n lazytxt = TL.toStrict (TL.take n lazytxt)
...
byteRange bs
  ...
  | otherwise = maxB - minB
  where
    (minB, maxB) = B.foldl' f (maxBound, 0) bs
    f (mn,mx) w = (min mn w, max mx w)
