...
incrementKey key pairs = [(k, if k == key then v + 1 else v) | (k, v) <- pairs]
...
freqs xs = foldr update Map.empty xs
  where
    update x m = Map.alter (Just . maybe 1 (+1)) x m
------------------------------------------------------------------------------
...
transfer from to amount bank
  ...
  | otherwise =
      let fromBalance = Map.findWithDefault 0 from bank
