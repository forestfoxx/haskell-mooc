...
combine op (Picture f1) (Picture f2) = Picture g
  where
    g coord = op (f1 coord) (f2 coord)
------------------------------------------------------------------------------
...
paintSolid color shape (Picture base) = Picture g
  where
    g coord = if contains shape (x coord) (y coord)
                then color
                else base coord
    x (Coord cx _) = cx
    y (Coord _ cy) = cy
...
paint (Picture pat) shape (Picture base) = Picture g
  where
    g coord = if contains shape (x coord) (y coord)
                then pat coord
                else base coord
...
instance Transform Blur where
  apply Blur (Picture f) = Picture avg
    where
      avg (Coord x y) = averageColors
        [ f (Coord x     y)
        , f (Coord (x-1) y)
        , f (Coord (x+1) y)
        , f (Coord x (y-1))
      ...
      averageColors cs = Color (r `div` n) (g `div` n) (b `div` n)
        where
          n = fromIntegral (length cs)
          (r,g,b) = foldr (\(Color r' g' b') (sr,sg,sb) ->
                            (sr + r', sg + g', sb + b')) (0,0,0) cs
